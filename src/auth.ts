...
export function helpBody(origin: string, env?: Env): unknown {
  ...
  return {
    ...
    sop: [
      ...
      `Clean up in bulk with POST /v1/cleanup: target ids or the list filters (expires=never, expires_within=24h|7d, expires_before, updated_before, min_size, q, created_by), action delete, set_ttl (with ttl), or expire (30m grace). Without confirm it is a dry run. Show the human the preview (matched, eligible, skipped, bytes, sample), then resend the same body with its confirm to execute. GET /v1/sites and GET /v1/files take the same filters plus sort=size|age to find candidates first. expires_within windows are computed at request time. GET /v1/export first if they need a copy of what they own; cleanup {} is involvement, which is wider than ownership.`,
    ...
    routes: {
      ...
      "GET /v1/files": "loose files you created or last wrote. ?scope=created|edited|involved&q=&created_by=&expires=never|expires_before=<iso>|expires_within=24h|7d&updated_before=<iso>&min_size=<bytes|500mb>&sort=updated|name|size|age&limit=25&cursor=. expires_within windows are computed at request time. Items carry last_read_at (floor; null = never)",
