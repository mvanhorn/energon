...
export function helpBody(origin: string, env?: Env): unknown {
  ...
  return {
    ...
    routes: {
      ...
      "GET /v1/files": "loose files you created or last wrote. ?scope=created|edited|involved&q=&created_by=&expires=never|expires_before=<iso>&updated_before=<iso>&min_size=<bytes|500mb>&sort=updated|name|size|age|last_read&changed_since_read=1&limit=25&cursor=. Items carry last_read_at (floor; null = never). last_read is never-read first. changed_since_read=1 is updated after last_read_at (never-read counts as changed). Not a cleanup target.",
