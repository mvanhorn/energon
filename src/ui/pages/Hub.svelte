  ...
  <div class="en-stack en-space-after">
    <Card title="Catalog" hint={`${lists.items.length < lists.total ? `${lists.items.length} of ` : ''}${count(lists.total, 'item')}`} tight>
      ...
      <div id="catalog-filters" class="en-card-body en-catalog-filters" hidden={!filtersOpen}>
        ...
        <Field label="Last open" note="Updated after the last recorded read. Never-read counts as changed. The stamp is a floor." noteId="catalog-changed-since-read-note"><SegmentedControl id="catalog-changed-since-read" ariaLabel="Changed since last open" options={[{ value: 'any', label: 'Any' }, { value: 'changed', label: 'Changed since last open' }]} bind:value={changedSince} onChange={() => { applyFilters(); }} /></Field>
      </div>
